...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create chimera-1bba
  fly apps create chimera-1bba-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app chimera-1bba
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app chimera-1bba-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'chimera-1bba-staging' does not match app name in config file 'chimera-1bba'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name chimera-1bba-db
  fly postgres attach --app chimera-1bba chimera-1bba-db

  fly postgres create --name chimera-1bba-staging-db
  fly postgres attach --app chimera-1bba-staging chimera-1bba-staging-db
  ```
